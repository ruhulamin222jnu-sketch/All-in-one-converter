...
def excel_to_pdf():
    ...
    df = pd.read_excel(filepath) if filename.lower().endswith((".xls", ".xlsx")) else pd.read_csv(filepath)
    pdf_path = os.path.join(app.config['DOWNLOAD_FOLDER'], filename.rsplit('.',1)[0]+'.pdf')
    ...
    for col in df.columns:
        pdf.cell(0, 10, txt=str(col), ln=1)
    for index, row in df.iterrows():
        pdf.cell(0, 10, txt=str(row.values), ln=1)
    pdf.output(pdf_path)
    return send_file(pdf_path, as_attachment=True)
